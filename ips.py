...
def calcula_ips(ip_base, lista_lista_hosts, target_location):
    ...
    with open(target_location, 'w') as wr:
        for mask in lista_mascaras:
            while True:
                rango_subneteado = list(network.subnets(new_prefix=int(mask)))#lista_mascaras
                rango_limpio = rango_subneteado[:int(num_redes[contador])]#num_base
                ...
                while contador2 < num_redes[contador]:
                    ...
                    rango_subneteado = list(network.subnets(new_prefix=int(mask)))
                contador = contador + 1
                break
            wr.write('\n')
